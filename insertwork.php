...
if(filter_input(INPUT_POST, 'year')){
   ...
   if ($authority['role']=="教师"){
    ...
    $statement = "INSERT INTO work(id, year, teacherId, typeName, rankName, content, amount, classHour,money,recorder,status) VALUES (null, :year, :teacherId, :typeName, :rankName,:content, :amount, :classHour,:money,:recorder,:status)";
...
$name = "";
//查询🌩大类
$queryType1 = "select distinct bigType from worktype order by bigType";
$queryTypeObject1 = $pdo->prepare($queryType1);
$queryTypeObject1->execute();
$bigType1 = $queryTypeObject1->fetchAll();

$queryType2 = "select distinct typeName from worktype order by typeName";
$queryTypeObject2 = $pdo->prepare($queryType2);
$queryTypeObject2->execute();
$bigType2 = $queryTypeObject2->fetchAll();

$queryType3 = "select distinct rank from worktype order by rank";
$queryTypeObject3 = $pdo->prepare($queryType3);
$queryTypeObject3->execute();
$bigType3 = $queryTypeObject3->fetchAll();
?>
<script>
...
function pickName()
{
    window.open("pickname.php","_blank",
        "height=500,width=500,left=300,top=500,location=no,"+
					...
					<div class="panel-body">
						<form role="form" id="workform" action="insertwork.php" method='post'>
							...
							<div class="form-group input-group">
								...
								<select class="form-control" name="typeName" id="typeNameOption" ><!-- onchange="selectRank()"  -->
									<?php 
									echo"<option value =''>--请选择类别--</option>";
									foreach ($bigType2 as $row){
										echo "<option value ='".$row['typeName']."'".(empty($year)?"selected":'').">".$row['typeName']."</option>";
									}
									?>
								</select>
							</div>
							<div class="form-group input-group">
								<span class="input-group-addon"><i class="fa fa-level-up">
										&nbsp;级别</i></span> 
								<select class="form-control" name="rankName" id="rankNameOption"><!--  onchange="getLmitRange()" -->
									<?php 
									echo"<option value =''>--请选择级别--</option>";
									foreach ($bigType3 as $row){
										echo "<option value ='".$row['rank']."'".(empty($year)?"selected":'').">".$row['rank']."</option>";
									}
									?>
								</select>
							</div>
							...
							<div class="form-group input-group">
								...
								<input type="text" class="form-control" id="classHour"   maxlength="10"  name='classHour'  onkeyup="checkRange(this)"/>
							</div>
							<div class="form-group input-group">
								<span class="input-group-addon"><i class="fa fa-yen"> &nbsp;奖励金额</i></span>
								<input type="text" class="form-control" id="money" 	name='money' maxlength="10"  onkeyup="checkRange(this)"/>
							</div>
